...
- **Anesthesia Timeline CRUD Redesign (Nov 2025)**: Systematic implementation of full CRUD operations for all timeline swimlanes using React Query as single source of truth. Completed for Times (13 predefined markers stored in JSONB) and Events (text annotations in separate table). Each swimlane has dedicated mutation hooks with proper cache invalidation and security controls to prevent unauthorized data migration across records.
