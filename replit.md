...
- **Treatments tab** (added 2026-04): Per-session aesthetic treatment documentation (Botox, fillers, injectables). Each treatment has multiple lines (service + product + dose + zones + lot + price), provider signature, and a manual "Create Invoice Draft" action that emits an editable `clinicInvoices` row. Mirrors the IntraOp medications palette/dose pattern. See `docs/superpowers/specs/2026-04-16-treatments-design.md`.
