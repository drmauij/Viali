...
- **Item Lifecycle Management**: Creation, updating, and transactional cascade deletion of items ensuring data integrity across related records (alerts, activities, order lines, lots, stock levels). Items support image uploads with automatic compression (max 800px, 0.8 quality JPEG) for visual identification, stored as base64 in the database. Edit dialog uses a tabbed interface with "Item Details" and "Item Photo" sections for better organization (matching the Controlled Substances page pattern).
