# Card Reader Bridge Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a Python system tray app that reads Swiss eCH-0064 insurance cards and opens the matching patient in Viali.

**Architecture:** Single-file Python app (`bridge.py`) with three threads: card polling, API communication, and system tray UI. Packaged as Windows .exe via PyInstaller.

**Tech Stack:** pyscard, pystray, Pillow, requests, PyInstaller

---

### Task 1: Update requirements.txt and config template
...
Replace contents of `card-reader/requirements.txt` with:
...
**Step 2: Update config.env.template with correct URL**

Replace contents of `card-reader/config.env.template` with:

```
# Viali Card Reader Bridge Configuration
# Copy this file to config.env and fill in your values

# ============================================
# VIALI API CONNECTION
# ============================================
# Base URL of the Viali webapp
VIALI_URL=https://use.viali.app

# Card reader API token (generate in Viali Admin > Hospital Settings > Card Reader)
VIALI_TOKEN=your-card-reader-token-here

# ============================================
# READER SETTINGS
# ============================================
# Preferred reader name substring (leave empty to use first available reader)
# Example: "ACS ACR122" or "Cherry" — matches partial names
PREFERRED_READER=

# Card polling interval in seconds (how often to check for card insert/remove)
POLL_INTERVAL_SECONDS=1
```

**Step 3: Install new dependencies**

Run: `cd /home/mau/viali/card-reader && source venv/bin/activate && pip install pystray Pillow requests`

**Step 4: Commit**

```bash
git add card-reader/requirements.txt card-reader/config.env.template
git commit -m "chore(card-reader): update dependencies and config template"
```

---

### Task 2: Write bridge.py — config loading and card reading

This is the core of the bridge. We build it in one file, section by section.

**Files:**
- Create: `card-reader/bridge.py`

**Step 1: Write bridge.py with config loading, card reading, TLV parsing, API client, and tray UI**

Create `card-reader/bridge.py` with the complete implementation below. The file has these sections:

1. **Config loading** — reads `config.env` from same directory as the script/exe
2. **TLV parsing** — decodes eCH-0064 EF.ID and EF.AD fields
3. **Card reader** — polls for card insertion, reads patient data via APDU
4. **API client** — POSTs to Viali lookup endpoint, opens browser
5. **System tray** — colored icon, tooltip, right-click menu
...
def parse_ech0064_tlv(data, field_defs):
    ...
    Returns dict of field_name -> decoded string value.
    """
    result = {}
    raw = bytes(data)
...
def read_card_data(connection):
    ...
    def read_binary(length):
        # Extended APDU for lengths > 255, short for <=255
        if length <= 255:
...
class CardReaderBridge:
        ...
        while self.running:
            try:
                ...
                if card_present and not card_was_present:
                    ...
                    # No card, make sure we show ready status
    ...
    def run(self):
        ...
        if not token or token == "your-card-reader-token-here":
            ...
            # Still run so user can see the tray icon with error
...
**Step 2: Test it locally (with card reader connected)**

Run: `cd /home/mau/viali/card-reader && source venv/bin/activate && python bridge.py`

Expected: System tray icon appears. Insert card -> reads data and logs to console. If no VIALI_TOKEN configured, shows red icon with error message. Ctrl+C or right-click Quit to stop.

**Step 3: Commit**

```bash
git add card-reader/bridge.py
git commit -m "feat(card-reader): add bridge application

Reads Swiss eCH-0064 insurance cards via USB reader, POSTs patient
data to Viali API, and opens the patient page in the default browser.
Runs as a system tray app with status indicator."
```

---

### Task 3: Add Windows build script

**Files:**
- Create: `card-reader/build.bat`

**Step 1: Create build.bat for PyInstaller packaging**
...
```bash
git add card-reader/build.bat
git commit -m "chore(card-reader): add Windows build script"
```

---

### Task 4: Update config.env.template URL and test end-to-end

**Step 1: Create a config.env for local testing**

Copy `config.env.template` to `config.env` and fill in a real token (generated from Viali Admin > Hospital Settings > Card Reader).
...
Test checklist:
- [ ] Tray icon appears (green if token valid, red if not)
- [ ] Insert card -> console shows "Card detected, reading..."
- [ ] Console shows patient name and AHV number
- [ ] Browser opens to patient page (or new-patient form)
- [ ] Remove card -> console shows "Card removed", icon goes green
- [ ] Insert same card again within 5s -> debounce kicks in
- [ ] Insert same card after 5s -> reads again normally
- [ ] Right-click tray icon -> Quit works

**Step 3: Ensure config.env is gitignored**

Run: `grep -q "config.env" /home/mau/viali/.gitignore || echo "card-reader/config.env" >> /home/mau/viali/.gitignore`

**Step 4: Final commit**

```bash
git add .gitignore
git commit -m "chore: gitignore card-reader config.env"
```
