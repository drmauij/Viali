      ...
      hospitalLinks.map(async (link) => {
        ...
        return {
          ...
          response: response ? {
            ...
            patientStreet: response.patientStreet,
            patientPostalCode: response.patientPostalCode,
            patientCity: response.patientCity,
          } : undefined,
        };
      })
    );
...
router.get('/api/public/questionnaire/:token', questionnaireFetchLimiter, async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      patientStreet: patient?.street || existingResponse?.patientStreet,
      patientPostalCode: patient?.postalCode || existingResponse?.patientPostalCode,
      ...
      existingResponse: existingResponse ? {
        ...
        patientStreet: existingResponse.patientStreet,
        patientPostalCode: existingResponse.patientPostalCode,
        patientCity: existingResponse.patientCity,
  ...
  try {
    ...
    if (response) {
      ...
      response = await storage.updateQuestionnaireResponse(response.id, {
        ...
        patientStreet: data.patientStreet ?? response.patientStreet,
        patientPostalCode: data.patientPostalCode ?? response.patientPostalCode,
        patientCity: data.patientCity ?? response.patientCity,
...
router.post('/api/patient-portal/:token/create-update-link', updateQuestionnaireLimiter, async (req: Request, res: Response) => {
  try {
    ...
    await storage.createQuestionnaireResponse({
      ...
      patientStreet: sourceResponse.patientStreet,
      patientPostalCode: sourceResponse.patientPostalCode,
      patientCity: sourceResponse.patientCity,
