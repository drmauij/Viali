...
router.get('/api/public/booking/:bookingToken/promo/:code', async (req: import("express").Request, res: Response) => {
  try {
    ...
    // Task 12: widen the lookup when the booking hospital belongs to a
    // group — any promo issued with `groupWide = true` at a sibling hospital
    // is also valid here. Single-hospital codes still match only when issued
    // at this hospital.
    const groupId = await getHospitalGroupIdCached(hospital.id, req);
    const groupHospitalIds = groupId
      ? await getGroupHospitalIdsCached(groupId, req)
      : [hospital.id];

    const lookupCond =
      groupHospitalIds.length > 1
        ? or(
            eq(promoCodes.hospitalId, hospital.id),
            and(
              eq(promoCodes.groupWide, true),
              inArray(promoCodes.hospitalId, groupHospitalIds),
            ),
          )!
        : eq(promoCodes.hospitalId, hospital.id);
    ...
    const rows = await db
      .select()
      .from(promoCodes)
      .where(and(eq(promoCodes.code, code), lookupCond))
      .orderBy(
        // Prefer the hospital-local row when both exist so branding/valid
        // dates matching the booking hospital win ties.
        sql`CASE WHEN ${promoCodes.hospitalId} = ${hospital.id} THEN 0 ELSE 1 END`,
      )
      .limit(1);
...
router.post('/api/public/booking/:bookingToken/book', async (req, res) => {
  try {
    ...
    try {
      ...
      // Increment promo code usage if provided. Task 12: widen the match to
      // include group-wide codes issued at any sibling hospital, mirroring
      // the validation endpoint. Without this a group-wide redemption would
      // not update its parent row's usedCount.
      if (req.body.promoCode) {
        try {
          const { promoCodes } = await import("@shared/schema");
          const promoGroupId = await getHospitalGroupIdCached(hospital.id, req);
          const promoGroupHospitalIds = promoGroupId
            ? await getGroupHospitalIdsCached(promoGroupId, req)
            : [hospital.id];
          const incCond =
            promoGroupHospitalIds.length > 1
              ? or(
                  eq(promoCodes.hospitalId, hospital.id),
                  and(
                    eq(promoCodes.groupWide, true),
                    inArray(promoCodes.hospitalId, promoGroupHospitalIds),
                  ),
                )!
              : eq(promoCodes.hospitalId, hospital.id);
...
router.post('/api/clinic/:hospitalId/units/:unitId/appointments', isAuthenticated, requireStrictHospitalAccess, requireWriteAccess, async (req: any, res) => {
  try {
    ...
    const { referralSource, referralSourceDetail, referralCreatedAt, metaLeadId, metaFormId, sendConfirmation, ...appointmentBody } = req.body;
    ...
    if (appointment.patientId && sendConfirmation !== false) {
...
export async function importLeadReferralHandler(req: any, res: Response): Promise<void> {
  try {
    const { hospitalId, appointmentId } = req.params;
    ...
    if (appointment.appointmentType === "internal") {
      res.status(409).json({ message: "Referral does not apply to internal appointments" });
      return;
    }
    if (!appointment.patientId) {
      res.status(400).json({ message: "Appointment has no patient" });
      return;
    }
