...
// =====================================================
// HIN Database Batch Matching Endpoints
// =====================================================

// Get HIN match status for a hospital - categorized by status
router.get('/api/hin-matches/:hospitalId', isAuthenticated, async (req: any, res) => {
  try {
    const { hospitalId } = req.params;
    ...
    let query = db
      .select({
        id: itemHinMatches.id,
        itemId: itemHinMatches.itemId,
        matchStatus: itemHinMatches.matchStatus,
        matchMethod: itemHinMatches.matchMethod,
        matchConfidence: itemHinMatches.matchConfidence,
        matchReason: itemHinMatches.matchReason,
        hinPharmacode: itemHinMatches.hinPharmacode,
        hinGtin: itemHinMatches.hinGtin,
        hinDescriptionDe: itemHinMatches.hinDescriptionDe,
        hinPexf: itemHinMatches.hinPexf,
        hinPpub: itemHinMatches.hinPpub,
        hinSmcat: itemHinMatches.hinSmcat,
        originalPharmacode: itemHinMatches.originalPharmacode,
        originalGtin: itemHinMatches.originalGtin,
        itemName: itemHinMatches.itemName,
        lastMatchAttempt: itemHinMatches.lastMatchAttempt,
        verifiedAt: itemHinMatches.verifiedAt,
        appliedAt: itemHinMatches.appliedAt,
      })
      .from(itemHinMatches)
      .where(eq(itemHinMatches.hospitalId, hospitalId))
      .orderBy(desc(itemHinMatches.lastMatchAttempt));

    const matches = await query;

    // Group by status
    const grouped = {
      matched: matches.filter(m => m.matchStatus === 'matched'),
      toVerify: matches.filter(m => m.matchStatus === 'to_verify'),
      unmatched: matches.filter(m => m.matchStatus === 'unmatched'),
      rejected: matches.filter(m => m.matchStatus === 'rejected'),
      counts: {
        matched: 0,
        toVerify: 0,
        unmatched: 0,
    ...
    grouped.counts.matched = grouped.matched.length;
    grouped.counts.toVerify = grouped.toVerify.length;
    grouped.counts.unmatched = grouped.unmatched.length;
    grouped.counts.rejected = grouped.rejected.length;

    // If specific status requested, return only that
    if (status) {
      const statusMap: Record<string, any[]> = {
        'matched': grouped.matched,
        'to_verify': grouped.toVerify,
        'unmatched': grouped.unmatched,
        'rejected': grouped.rejected,
      };
      res.json({ items: statusMap[status as string] || [], counts: grouped.counts });
    } else {
      res.json(grouped);
    }
  } catch (error) {
    console.error("Error fetching HIN matches:", error);
    res.status(500).json({ message: "Failed to fetch HIN matches" });
  }
});

// Trigger batch HIN sync for a hospital
router.post('/api/hin-matches/:hospitalId/sync', isAuthenticated, requireWriteAccess, async (req: any, res) => {
  try {
    const { hospitalId } = req.params;
    ...
    // Import and run the batch sync
    const { batchMatchHospitalItemsToHin } = await import('../services/hinMediupdateClient');
    
    // Run sync (this could take a while for large inventories)
    const result = await batchMatchHospitalItemsToHin(hospitalId);
    ...
    console.error("Error running HIN batch sync:", error);
    res.status(500).json({ message: "Failed to run HIN batch sync" });
  }
});

// Approve a fuzzy HIN match - apply HIN data to item codes
router.post('/api/hin-matches/:matchId/approve', isAuthenticated, requireWriteAccess, async (req: any, res) => {
  try {
    const { matchId } = req.params;
    const userId = req.user.id;

    // Import and apply the match
    const { applyHinMatchToItem } = await import('../services/hinMediupdateClient');
    const result = await applyHinMatchToItem(matchId, userId);

    if (!result.success) {
      return res.status(400).json({ message: result.error || 'Failed to apply match' });
    }

    res.json({ success: true });
  } catch (error) {
    console.error("Error approving HIN match:", error);
    res.status(500).json({ message: "Failed to approve HIN match" });
  }
});

// Reject a fuzzy HIN match
router.post('/api/hin-matches/:matchId/reject', isAuthenticated, requireWriteAccess, async (req: any, res) => {
  try {
    const { matchId } = req.params;
    const userId = req.user.id;

    // Import and reject the match
    const { rejectHinMatch } = await import('../services/hinMediupdateClient');
    const result = await rejectHinMatch(matchId, userId);

    if (!result.success) {
      return res.status(400).json({ message: result.error || 'Failed to reject match' });
    }

    res.json({ success: true });
  } catch (error) {
    console.error("Error rejecting HIN match:", error);
    res.status(500).json({ message: "Failed to reject HIN match" });
  }
});
