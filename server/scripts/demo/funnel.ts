...
export async function seedFunnelData(args: {
  ...
  for (const window of ["current", "prior", "older"] as const) {
    for (let hIdx = 0; hIdx < locationRows.length; hIdx++) {
      ...
      const leadCnt = await seedLeadsForClinic({ loc, prng, count: leadCount, window, clinicPatients });
      stats.leads += leadCnt;

      console.log(
        `  > ${loc.hospital.name} [${window}] referrals=${r.referrals} appts=${r.appointments} tx=${r.treatments} leads=${leadCnt}`,
      );
    }
  }
...
async function seedLeadsForClinic(args: {
  ...
  const { loc, prng, count, window, clinicPatients } = args;
  const period = PERIOD_BOUNDS[window];
  for (let i = 0; i < count; i++) {
    const src = prng.weighted(SOURCE_WEIGHTS);
    const leadDate = prng.dateInRange(period.from, period.to);
    // Lead patient is sometimes a known patient (~30%), sometimes anonymous.
    const known = clinicPatients.length > 0 && prng.next() < 0.3;
    const patient = known ? prng.pick(clinicPatients) : null;
    await db.insert(leads).values({
      hospitalId: loc.hospital.id,
      source: src.utmSource, // freeform string on leads
      utmSource: src.utmSource,
      utmMedium: src.utmMedium,
