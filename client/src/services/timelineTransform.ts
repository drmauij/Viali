...
/**
 * Transform medications into infusion markers (ticks on timeline)
 * This includes BOTH active and stopped infusions
 */
export function transformInfusionMarkers(
  medications: any[],
  itemToSwimlane: Map<string, string>,
  anesthesiaItems: AnesthesiaItem[]
): { [swimlaneId: string]: [number, string][] } {
  const markers: { [swimlaneId: string]: [number, string][] } = {};
  ...
  Object.entries(recordsByLane).forEach(([swimlaneId, records]) => {
    ...
    startRecords.forEach(startRec => {
      ...
      // Only process free-flow infusions
      if (!item || (item.rateUnit !== 'free' && startRec.rate !== 'free')) {
        return;
      }
      ...
      if (!markers[swimlaneId]) {
        markers[swimlaneId] = [];
      }
      
      // Add start marker with dose value
      markers[swimlaneId].push([startTime, startRec.dose || '?']);
      
      // Add stop marker if infusion was stopped
      if (startRec.endTimestamp) {
        const endTime = new Date(startRec.endTimestamp).getTime();
        markers[swimlaneId].push([endTime, '']); // Empty string indicates stop marker
      }
    });
    
    // Sort markers by timestamp
    if (markers[swimlaneId]) {
      markers[swimlaneId].sort((a, b) => a[0] - b[0]);
    }
  });
