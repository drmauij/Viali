...
export default function ClinicCalendar({ 
  ...
  const calendarEvents: CalendarEvent[] = useMemo(() => {
    ...
    const appointmentEvents = appointments.map((appt) => {
      ...
      if (actualStart) {
        // Use actual start time, but keep the original appointment date
        const actualStartDate = new Date(actualStart);
        start = new Date(appointmentDate);
        start.setHours(actualStartDate.getHours(), actualStartDate.getMinutes(), 0, 0);
      } else {
      ...
      if (actualEnd) {
        // Use actual end time, but keep the original appointment date
        const actualEndDate = new Date(actualEnd);
        end = new Date(appointmentDate);
        end.setHours(actualEndDate.getHours(), actualEndDate.getMinutes(), 0, 0);
      } else {
                ...
                return (
                  <div className="w-full flex items-center justify-center gap-1.5 py-2 px-1" data-testid={`provider-header-${resource.id}`}>
                    <button
                      ...
                      className="flex items-center gap-1 hover:bg-muted/50 rounded transition-colors cursor-pointer font-medium text-sm px-1"
      ...
      <Dialog open={!!pendingReschedule} onOpenChange={(open) => { if (!open) handleCancelReschedule(); }}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="flex flex-col gap-2 pt-2">
            <Button onClick={() => handleConfirmReschedule(true)} className="w-full">
              {t('appointments.rescheduleAndNotify', 'Reschedule & Notify')}
            </Button>
            <Button variant="secondary" onClick={() => handleConfirmReschedule(false)} className="w-full">
              {t('appointments.rescheduleOnly', 'Reschedule Only')}
            </Button>
            <Button variant="outline" onClick={handleCancelReschedule} className="w-full">
