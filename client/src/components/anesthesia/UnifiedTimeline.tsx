...
export function UnifiedTimeline({
  ...
  const option = useMemo(() => {
    ...
    const yAxes = [
      // Vitals grid - First y-axis (BP/HR: 0 to 260 range for top and bottom padding, showing 20-220 labels)
      {
        type: "value" as const,
        gridIndex: 0,
        min: 0,
    ...
    // First y-axis (20-220, interval 20) - positioned at x=140px, grid extends 0 to 260 for top and bottom padding
    for (let val = 20; val <= 220; val += 20) {
      const yPercent = ((260 - val) / 260) * 100; // Invert because top is 0, using 260 range (0 to 260)
    ...
    <div className="w-full relative" style={{ height: componentHeight }}>
      ...
      <div className="absolute left-0 top-0 w-[200px] h-full border-r border-border z-30 bg-background">
        ...
        <div className="absolute top-[32px] h-[340px] w-full pointer-events-none z-50">
          {/* First scale: 20-220 with 20-unit steps (11 values) - close to grid, grid extends 0 to 260 for top and bottom padding */}
          {[20, 40, 60, 80, 100, 120, 140, 160, 180, 200, 220].map((val) => {
            const yPercent = ((260 - val) / 260) * 100;
            return (
              <div 
                key={`scale1-${val}`}
        ...
        <div
          ...
          onMouseMove={(e) => {
            ...
            if (activeToolMode === 'edit' && selectedPoint) {
              ...
              // BP/HR scale: 0 to 260 (only show if within editable window)
              const minVal = 0;
              const maxVal = 260;
              value = Math.round(maxVal - (yPercent * (maxVal - minVal)));
