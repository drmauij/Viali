...
/**
 * Build itemId to swimlaneId mapping
 * This maps each inventory item to its corresponding timeline swimlane
 */
function buildItemToSwimlaneMap(anesthesiaItems: AnesthesiaItem[], administrationGroups: AdministrationGroup[]): Map<string, string> {
  const map = new Map<string, string>();
  
  anesthesiaItems.forEach(item => {
    if (item.administrationGroup) {
      // Swimlane ID format: "group_{groupId}_item_{itemId}"
...
/**
 * Transform flat medication records into medicationDoseData state
 * Only processes bolus-type medications
 */
function transformMedicationDoses(
  medications: any[],
  ...
  medications
    ...
    .forEach(med => {
      ...
      if (!swimlaneId) return; // Skip if item not configured
...
/**
 * Transform infusion records into rate infusion sessions
 * Reconstructs ACTIVE session from infusion_start, rate_change, and stop records
 * Uses infusion_stop records to determine if an infusion has been stopped
 */
function transformRateInfusions(
  medications: any[],
  itemToSwimlane: Map<string, string>,
  ...
  // Group infusion-related records by swimlane
  ...
  // Reconstruct ACTIVE session for each swimlane
  // Note: This currently only returns the most recent/active session
  // Historical completed sessions are not yet rendered in the timeline
  Object.entries(infusionsByLane).forEach(([swimlaneId, records]) => {
    // Sort by timestamp
    records.sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
    
    // Find the most recent infusion_start
    const startRecords = records.filter(r => r.type === 'infusion_start');
    if (startRecords.length === 0) return; // No infusions
    ...
    if (!item || !item.rateUnit || item.rateUnit === 'free') return; // Only rate-controlled infusions
    
    const startTime = new Date(startRecord.timestamp).getTime();
    
    // Check for matching infusion_stop record or endTimestamp on start record
    ...
    // Build segments from start + subsequent rate_change records within the infusion window
    const segments: any[] = [];
    ...
    // Add rate changes that occurred after start but before stop (if stopped)
    records
      .filter(r => {
    ...
    sessions[swimlaneId] = {
      swimlaneId,
      label: item.name,
      syringeQuantity: startRecord.dose || '50ml', // Use dose field for syringe quantity
      segments,
      state,
    };
  });
...
/**
 * Transform free-flow infusion records into freeFlowSessions state
 * Note: Currently only returns ACTIVE (non-stopped) free-flow sessions
 * Stopped sessions are filtered out based on infusion_stop records or endTimestamp
 */
function transformFreeFlowInfusions(
  medications: any[],
  itemToSwimlane: Map<string, string>,
  anesthesiaItems: AnesthesiaItem[]
): { [swimlaneId: string]: any[] } {
  ...
  // Group all infusion records by swimlane and itemId
  ...
  Object.entries(recordsByLane).forEach(([swimlaneId, records]) => {
    ...
    startRecords.forEach(startRec => {
      ...
      if (!item || item.rateUnit !== 'free') return; // Only free-flow infusions
      
      const startTime = new Date(startRec.timestamp).getTime();
      
      // Check if this infusion has been stopped
      const hasEndTimestamp = !!startRec.endTimestamp;
      const stopRecord = records.find(r => 
        r.type === 'infusion_stop' && new Date(r.timestamp).getTime() > startTime
      );
      
      // Skip stopped infusions (for now, only show active sessions)
