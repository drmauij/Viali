...
export default function DayNotesPanel({ hospitalId, selectedDate }: DayNotesPanelProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (value: string) => {
      const res = await apiRequest(
        'PUT',
        `/api/op-day-notes/${hospitalId}/${dateString}`,
        { notes: value },
      );
      return res.json();
    },
  ...
  // Safety net: flush on unmount / date change when dirty so nothing is lost.
  const notesRef = useRef(notes);
  notesRef.current = notes;
  useEffect(() => {
    return () => {
      if (notesRef.current !== remoteRef.current) {
        apiRequest('PUT', `/api/op-day-notes/${hospitalId}/${dateString}`, {
